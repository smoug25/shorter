...
import Data.Time ( addDays, getCurrentTime, Day, UTCTime(utctDay) )
import Db (withConn)
import Database.PostgreSQL.Simple
    ( Connection, execute, query, Only(Only), FromRow )
...
instance Show Url where
    show url = mconcat [ show $ urlId url
                        , ".) "
                        , "\nhash: "
                        , urlHash url
                        , "\norigin url: "
...
updateUserOrWarn conn (Just user) =  do
    let q = mconcat [ "UPDATE users SET "
                    , "last_login = ?,"
                    , "WHERE user_id = ?;"]
...
addUrl conn url user private exp = do
        execute conn "INSERT INTO urls (origin_url, hash, private, user_id, expiration_date) VALUES (?, ?, ?, ?, ?)" 
...
getUserUrls conn userEmail = 
    query conn 
        "SELECT * FROM urls WHERE user_id = (SELECT user_id from users where email = ?)" (Only userEmail) :: IO [Url]        